...
//  Remider
//
//  Created by Murilo Alves on 18/03/26.
//
...
class NewReceiptViewModel {
    ...
    private func scheduleNotifications(remedy: String, time: String, recurrence: String){
        ...
        guard let interval = getIntervalInHours(recurrence: recurrence) else {
            return
        }
        ...
        for i in 0..<(24 / interval){
            ...
            center.add(request) { error in
                if let error = error {
                    ...
                    print("Notificaco criada com sucesso")
                }
            }
    ...
    private func getIntervalInHours(recurrence: String) -> Int? {
        switch recurrence {
